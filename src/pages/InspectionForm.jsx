...
function InspectionForm() {
  const { id,asset_id,asset_name,emp_id,emp_name,dept_name} = useParams(); //extract id from the url
  const [inspectors, setInspectors] = useState([]);
  const [myAsset, setMyAsset] = useState("");
  const [currInspectot, setCurrInspectot] = useState("");
  ...
  useEffect(() => {
    if (asset_id && asset_name && emp_id && emp_name && dept_name) {
      setMyAsset(`${asset_id} - ${asset_name}`);
      setCurrInspectot(`${emp_id} - ${emp_name} - ${dept_name}`)
      setSplitDepartment(dept_name)
    }
    
  }, [asset_id,asset_name,emp_id,emp_name,dept_name]);
  ...
  useEffect(() => {
    console.log(myAsset.split(" - ")[0])
    const fetchDept = () => {
      if (myAsset && myAsset !=="") {
        axios
          .get(
            `${process.env.REACT_APP_BASE_URL2}/form/asset/formnames/${myAsset.split(" - ")[0]}`
          )
          .then((res) => {
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="form-group  mt-4">
        ...
        <div className="form-group">
          ...
          <div className="search-fields">
            <input type="text" list="asset_list" name="asset_details" value={myAsset} onChange={(e)=>{handleAssetChange(e.target.value)}}  className="form-control" readOnly required/>
        ...
        <div className="form-group">
          ...
          <div className="search-fields">
            {/* <input
              ...
              required
            /> */}
            <select name="inspection_form_name" onChange={handleFormName}required className="form-select" >
            <option>
                  Select Form
                </option>
